...
class GameState :
    ...
    def __init__(self) :
        ...
        self.player_spells = set()  # even though its a set, still have to ensure no duplicate spells with different
        # failure rates

        self.noise_level = None
        self.adjusted_noise_level = None

        self.general_knowledge_pddl_facts_filename = "D:\\Users\\Melody\\Google " \
                                                     "Drive\\morris85@students.rowan.edu\\Spring 2022\\AI\\FinalProject\\models\\general_dcss_knowledge_facts.pddl"
        self.general_knowledge_pddl_objects_filename = "D:\\Users\\Melody\\Google " \
                                                       "Drive\\morris85@students.rowan.edu\\Spring 2022\\AI\\FinalProject\\models\\general_dcss_knowledge_objects.pddl"
    ...
    def _process_single_spell(self, message) :
        ...
        spell_name_regex = re.compile(
            '(Absolute Zero|Agony|Airstrike|Alistair\'s Intoxication|Animate Dead|Animate Skeleton|Apportation|Aura '
            'of Abjuration|Beastly Appendage|Blade Hands|Blink|Bolt of Magma|Borgnjor\'s Revivification|Borgnjor\'s '
            'Vile Clutch|Call Canine Familiar|Call Imp|Cause Fear|Chain Lightning|Confusing Touch|Conjure Ball '
            'Lightning|Conjure Flame|Controlled Blink|Corona|Corpse Rot|Dazzling Flash|Death Channel|Death\'s '
            'Door|Discord|Disjunction|Dispel Undead|Dispersal|Dragon Form|Dragon\'s Call|Ensorcelled '
            'Hibernation|Eringya\'s Noxious Bog|Excruciating Wounds|Fire Storm|Fireball|Foxfire|Freeze|Freezing '
            'Cloud|Frozen Ramparts|Fulminant Prism|Gell\'s Gravitas|Hailstorm|Haunt|Hydra Form|Ice Form|Ignite '
            'Poison|Ignition|Infestation|Infusion|Inner Flame|Invisibility|Iron Shot|Irradiate|Iskenderun\'s '
            'Battlesphere|Iskenderun\'s Mystic Blast|Leda\'s Liquefaction|Lee\'s Rapid Deconstruction|Lehudib\'s '
            'Crystal Spear|Lesser Beckoning|Lightning Bolt|Magic Dart|Malign Gateway|Mephitic Cloud|Metabolic '
            'Englaciation|Monstrous Menagerie|Necromutation|Olgreb\'s Toxic Radiance|Orb of Destruction|Ozocubu\'s '
            'Armour|Ozocubu\'s Refrigeration|Pain|Passage of Golubria|Passwall|Petrify|Poisonous Vapours|Portal '
            'Projectile|Recall|Ring of Flames|Sandblast|Searing Ray|Shadow Creatures|Shatter|Shock|Shroud of '
            'Golubria|Silence|Simulacrum|Slow|Song of Slaying|Spectral Weapon|Spellforged Servitor|Spider '
            'Form|Starburst|Static Discharge|Statue Form|Sticks to Snakes|Sticky Flame|Sting|Stone Arrow|Sublimation '
            'of Blood|Summon Demon|Summon Forest|Summon Greater Demon|Summon Guardian Golem|Summon Horrible '
            'Things|Summon Hydra|Summon Ice Beast|Summon Lightning Spire|Summon Mana Viper|Summon Small '
            'Mammal|Swiftness|Teleport Other|Tornado|Tukima\'s Dance|Vampiric Draining|Yara\'s Violent Unravelling)')
    ...
    def _process_single_ability(self, message) :
        ...
        try :
            ...
            ability_obj = Ability(ability_name, ability_fail_rate, 'MP' in ability_costs, 'Piety' in ability_costs,
                                  'Delay' in ability_costs, 'Frailty' in ability_costs)
    ...
    def process_messages(self, data) :
        ...
        for m in data :
            ...
            if 'No target in view!' in message_only :
                self.cannot_fight = True

            if 'No reachable target in view!' in message_only :
                self.cannot_fight = True
            ...
            if 'leading down here.' in message_only :
        ...
        if self.player_current_hp == 0 :
            ...
            for i in range(1, len(ascending_bin_labels) + 1) :
                if self.player_current_hp < i * bin_size :
                    ...
                    print(
                        "Just wrote player_health to be {} because its value is {}".format(ascending_bin_labels[i - 1],
                                                                                           self.player_current_hp))
                    break
    ...
    def is_exploration_done(self, reset=True) :
        exploration_finished = self.exploration_done
        if reset :
    ...
    def can_go_down(self, reset=True) :
        going_down = self.stairs_down
        if reset :
            ...
            for cell_dict in cells :
                ...
                for datum_key in CellRawStrDatum :
                    if datum_key.name in cell_dict.keys() :
                        # input("datum_key {} is in cell_dict {} with value {}".format(datum_key.name, cell_dict,
                        # cell_dict[datum_key.name]))
